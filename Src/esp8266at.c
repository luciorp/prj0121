...
void	Wifi_HWRestart(void){
	...
	HAL_GPIO_WritePin(_WIFI_RESET_PORT, _WIFI_RESET_PIN, GPIO_PIN_RESET);
	Delay(100);
	HAL_GPIO_WritePin(_WIFI_RESET_PORT, _WIFI_RESET_PIN, GPIO_PIN_SET);
...
bool	Wifi_SendRaw(uint8_t *data,uint16_t len){
	...
	if(len <= _WIFI_TX_SIZE)
	{
		...
		if(HAL_UART_Transmit(&_WIFI_USART,data,len,1000) == HAL_OK)
			return true;
		else
			return false;
	}
